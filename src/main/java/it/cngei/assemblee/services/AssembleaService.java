...
public class AssembleaService {
    ...
    public void checkIsAdminOrCovepo(Long idAssemblea, Long idUtente) {
        var assemblea = getAssemblea(idAssemblea);
        if (!(
                Objects.equals(idUtente, assemblea.getIdProprietario()) ||
                        Objects.equals(idUtente, assemblea.getIdPresidente()) ||
                        (assemblea.getCovepo() != null && Arrays.asList(assemblea.getCovepo()).contains(idUtente)))) {
            throw new AccessDeniedException("Non sei un amministratore di questa assemblea");
        }
    }
